...
async function getChromeVersionWin(includeChromium) {
  ...
  const versions = contents.filter((a) =>
...
function getChromeVersionFromOsa(includeChromium) {
  ...
  try {
    const version = execSync(
      'osascript -e \'tell application "Chromium" to get version\''
    )
      .toString()
      .trim();
    return version;
...
export async function innerGetChromeVersion(includeChromium = false) {
 ...
 */
export async function getChromeVersion(includeChromium = false) {
